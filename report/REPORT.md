...
### 3.2 워크로드
- **uniform**: 4KB 랜덤쓰기를 SSD 용량의 약 3.3배만큼 반복(fio, `loops=250`). 핫/콜드 구분이 없는 균일 워크로드. 파일 크기(=디바이스 사용률)를 600 MiB / 22 GiB / 38 GiB로 바꿔가며 총 쓰기량은 146\~154 GiB로 고정한 **사용률 스윕**도 함께 측정했다(4.3.1절).
- **zipf**: 22 GiB 파일에 4KB 랜덤쓰기를 7회 반복(총 154 GiB)하되 접근 빈도가 멱법칙을 따르도록 `random_distribution=zipf:1.2` 지정. 소수의 페이지가 대부분의 쓰기를 받고, 꼬리 쪽 페이지는 거의 쓰이지 않는다.
- **zoned 80:20**: 위와 파일 크기·총 쓰기량이 동일하고 `random_distribution=zoned:80/20:20/80`만 다르다 — 쓰기의 80%가 파일 앞 20% 영역에 몰린다. zipf와 달리 "콜드" 영역도 균일하게 접근되므로 결국 전부 덮어써진다.
...
세부 지표로 들어가기 전에, 네 워크로드에서 실제로 블록들이 얼마나 닳았는지를 한 장으로 본다. 집계값이 아니라 실험 종료 시점에 `/proc/nvmev/debug`가 덤프한 131,072개 블록의 erase 횟수를 그대로 그린 것이다. 워크로드는 **Greedy와 Cost-Benefit의 victim 선택이 갈린 비율**이 낮은 것부터 높은 순으로 배치했다.

![그림 1 — 워크로드 × 정책별 블록 erase 횟수 분포와 erase된 블록 비율](figures/fig1_wear_distribution.png)

| 워크로드 | 선택이 갈린 비율 | Greedy 최대 마모 | Cost-Benefit 최대 마모 |
|---|---|---|---|
| uniform | 0% | 161 | 161 (완전 동일) |
| zoned 80:20 | 0% | 5 | 5 (완전 동일) |
| hotcold v7 | 90.3% | 10 | 9 |
| **zipf:1.2** | **98.4%** | **11** | **6** |

세 가지가 한눈에 드러난다.

- **두 정책의 선택이 갈리지 않는 워크로드에서는 결과도 완전히 같다.** uniform과 zoned 80:20에서 Greedy와 Cost-Benefit은 분포까지 겹친다. 이건 우연이 아니라 구조적으로 동일한 동작이며, 4.3절에서 실측으로 증명한다.
- **선택이 갈리는 워크로드에서만 Cost-Benefit의 이점이 나타나고, 갈리는 비율이 높을수록 효과도 크다.** hotcold(90.3%)에서 최대 마모가 10→9로, zipf(98.4%)에서는 11→6으로 줄었다.
- **Random은 모든 워크로드에서 마모를 가장 넓게 퍼뜨린다.** uniform에서 88.0%의 블록을 건드려 최대 마모를 161회에서 10회로 낮췄다. 다만 이건 공짜가 아니라 GC 비용을 크게 치른 결과이며, 그 대가는 4.1~4.4절의 migration/latency 지표에서 드러난다.

워크로드마다 기록한 총 바이트가 다르므로(uniform·zipf·zoned 146\~154 GiB, hotcold 102\~167 GiB) 워크로드 사이의 절대 높이를 직접 비교하는 것은 의미가 없다. 이 그림이 보여주는 것은 **같은 워크로드 안에서 정책별로 마모가 어떤 모양으로 퍼지는가**이다.

### 4.1 zipf — Cost-Benefit이 최대 마모를 절반 가까이 낮춘다 (핵심 결과)

가장 뚜렷한 차이는 zipf 분포(`random_distribution=zipf:1.2`)에서 나왔다. 22 GiB 파일에 4KB 랜덤쓰기를 7회 반복하며, 접근 빈도만 멱법칙을 따르도록 한 워크로드다. 세 정책 모두 `loops` 기반이라 **정확히 같은 154.0 GiB를 기록**하므로 정규화 없이 raw 값을 그대로 비교할 수 있다.

![그림 2 — zipf 최대 마모, 마모 균등도, migration 비용, p99 latency](figures/fig2_zipf_comparison.png)

| 지표 | Greedy | Cost-Benefit | Random |
|---|---|---|---|
| **최대 erase 횟수 (peak wear)** | 11.0 ± 1.0 | **6.0 ± 0.0** | 12.0 ± 0.0 |
| **erase 변동계수 (전체 블록)** | 0.677 ± 0.027 | **0.504 ± 0.032** | 0.735 ± 0.003 |
| erase된 블록 수 | 119,688 | **125,719** | 115,829 |
| GC migration 비용 (valid pages) | 928,521 | **918,232** | 2,621,154 |
| erase 총합 | 303,319 | 303,067 | 320,821 |
| latency 평균 | 42.9 μs | 42.9 μs | 45.7 μs |
| latency p99 | 97.8 μs | 100.5 μs | 131.6 μs |

(각 정책 3회 반복, 평균 ± 표준편차)

**Cost-Benefit은 최대 마모를 11.0에서 6.0으로 45.5% 낮추면서, migration 비용은 오히려 1.1% 적었다.** 반복측정 원시값이 완전히 분리된다 — Greedy는 `[10, 11, 12]`, Cost-Benefit은 3회 모두 정확히 `[6, 6, 6]`이었다. 변동계수도 겹치지 않는다(Greedy 최저 0.646 vs CB 최고 0.527).

즉 이 워크로드에서 Cost-Benefit은 **웨어 레벨링을 크게 개선하면서 그 대가를 사실상 치르지 않았다.** latency도 평균은 동일하고 p99만 2.8% 높은 수준이다.

작동 원리는 진단 카운터에 그대로 드러난다. Cost-Benefit으로 구동할 때, 같은 순간에 Greedy가 골랐을 line의 평균 vpc는 5.95인데 Cost-Benefit이 실제로 고른 line의 평균 vpc는 12.12였다. **Cost-Benefit은 당장 옮길 페이지가 2배나 많은(= 그만큼 오래되고 차가운) line을 일부러 골랐다.** 매 GC로 보면 손해지만, 그렇게 회수한 콜드 line이 오래 재사용되지 않는 덕에 마모가 넓게 퍼지고, 결과적으로 총 migration 비용은 늘지 않았다. 두 정책의 선택은 전체 GC의 **98.4%**에서 갈렸다.

**Random은 여기서도 가장 나쁘다.** migration 비용이 2.8배(2,621,154 vs 928,521), p99 latency가 1.35배이며, 최대 마모(12.0)마저 Greedy보다 나쁘다.

### 4.2 hotcold — 같은 경향이 다른 워크로드에서도 재현된다

![그림 3 — GiB당 migration 비용과 erase 횟수](figures/fig3_hotcold_efficiency.png)

![그림 4 — 최대 erase 횟수(peak wear)와 erase된 블록 수](figures/fig4_hotcold_wear_leveling.png)

![그림 5 — 호스트 IO 평균/p99 latency](figures/fig5_hotcold_latency.png)
...
### 4.3 uniform — 핫/콜드 구분이 없으면 두 정책은 "증명 가능하게" 동일하다

![그림 6 — migration 비용, erase 총합, 최댓값 (migration 비용은 GiB당이 아니라 raw 값이다 — uniform은 정책 무관하게 항상 정확히 같은 바이트 수를 기록하므로 정규화가 필요 없다)](figures/fig6_uniform_comparison.png)
...
#### 4.3.1 그런데 왜 후보가 전부 vpc=0이었나 — 사용률 스윕으로 검증
...
![그림 7 — uniform 사용률 스윕](figures/fig7_utilization_sweep.png)
...
#### 4.3.2 스큐를 줘도 수렴한다 — zoned 80:20

사용률이 원인이 아니라면 남는 후보는 접근 패턴이다. 그런데 **스큐를 주는 것만으로도 충분하지 않다.** zipf와 파일 크기·총 쓰기량을 똑같이 맞추고 분포만 `zoned:80/20:20/80`(쓰기의 80%가 앞 20% 영역)으로 바꾼 결과:

| 지표 | Greedy | Cost-Benefit | Random |
|---|---|---|---|
| GC 중 옮긴 valid page | **0** | **0** | 6,626,206 |
| erase 총합 | 292,748 | 292,748 (완전 동일) | 361,752 |
| erase 최댓값 | 5 | 5 | 12 |
| 두 정책의 선택이 갈린 비율 | **0%** | **0%** | — |

zoned 80:20에서도 두 정책은 완전히 동일했다. 원인은 "콜드"의 정의에 있다. zoned에서 콜드 영역(파일의 80%, 17.6 GiB)은 전체 쓰기의 20%인 30.8 GiB를 받으므로 **평균 1.75회씩 덮어써진다** — 느릴 뿐 결국 전부 무효화된다. 반면 zipf:1.2는 멱법칙 꼬리를 가지므로 **실행 내내 거의 쓰이지 않는 페이지가 실제로 존재**하고, 그 페이지들이 GC 시점까지 valid로 남아 victim에 "선택의 여지"를 만든다.

**따라서 갈림길은 "스큐가 있는가"가 아니라 "GC 시점까지 살아남는 진짜 콜드 데이터가 남는가"이다.** 네 워크로드가 이 기준으로 정확히 나뉜다 — uniform(0%)과 zoned 80:20(0%)은 모든 데이터가 결국 덮어써져 수렴하고, hotcold(90.3%)와 zipf(98.4%)는 콜드 데이터가 남아 갈린다.

이는 4.1의 zipf, 4.2의 hotcold와 정확히 대비된다 — hotcold에서는 두 정책이 GC의 90.3%에서 다른 line을 골랐고, 그 line들의 vpc 차이는 평균 33.8(평균 vpc의 47%)이었다. **Cost-Benefit의 이점은 회수 후보 사이에 실질적인 "선택의 여지"가 있을 때만 발현되며, 그 여지를 만드는 것은 용량이 아니라 스큐다.**
...
### 4.4 filebench — fio 결과 재확인 (보조 벤치마크)

![그림 9 — migration 비용, erase 효율(GiB당), erase 최댓값, erase된 블록 수](figures/fig9_filebench_comparison.png)
...
### 4.5 victim divergence 분석 — "다른 line을 골라도 비용은 비슷한가?"

![그림 10 — victim divergence 분석](figures/fig10_vpc_divergence.png)
...
![그림 11 — 힙 staleness 버그 수정 전후 비교](figures/fig11_bugfix_before_after.png)
...
1. **Cost-Benefit GC는 최대 마모를 낮추고 마모를 더 많은 블록에 분산시켜 웨어 레벨링을 개선한다.** zipf 워크로드에서는 최대 마모를 11.0에서 6.0으로 **45.5% 낮추면서 migration 비용은 오히려 1.1% 적었다**(4.1절). hotcold에서는 최대 마모를 10.3에서 8.3으로 낮추는 대신 migration 비용을 13.6% 더 썼다(4.2절). 웨어 레벨링 개선은 두 워크로드에서 일관되게 나타나고, 그 대가는 워크로드에 따라 없음에서 13.6% 사이로 달라진다.
2. **이 이점은 GC 시점까지 살아남는 콜드 데이터가 있는 워크로드에서만 나타난다.** 네 워크로드가 이 기준으로 정확히 갈렸다 — uniform과 zoned 80:20에서는 두 정책의 선택이 **단 한 번도** 갈리지 않아 결과가 완전히 동일했고(0% divergence), hotcold(90.3%)와 zipf(98.4%)에서는 갈렸으며 갈리는 비율이 높을수록 효과도 컸다. 원인이 디바이스가 비어서가 아님은 사용률 스윕으로 배제했다(4.3.1절) — 85%까지 채워 Random의 GC 비용이 2.9배로 오르는 동안에도 두 정책은 valid page를 하나도 옮기지 않았다. 스큐를 주는 것만으로도 부족하다(4.3.2절) — zoned 80:20은 스큐가 있지만 콜드 영역조차 평균 1.75회 덮어써져 결국 전부 무효화되므로 수렴한다. **갈림길은 용량도 스큐 자체도 아니라, 회수 시점까지 valid로 남아있는 데이터가 존재하는가이다.**
