...
### 2026-07-31 (벤치마크 방법론 최종 점검)
- 무엇을: 제출 전 워크로드 파라미터가 실제 디바이스 스케일에 적절한지 재검토하고, 빠져 있던 검증 2가지를 수행.
- **디바이스 기하 구조 실측 확정**: `/proc/nvmev/debug` 덤프 구조(`ns part ch lun pl blk cnt`)의 실제 인덱스 범위로 확인 — 8채널이 4개 파티션에 2개씩 분배되어, 파티션당 `2ch × 2lun × 1pl × 8192blk`. 즉 **전체 131,072 블록 / 32,768 line, line 하나 = 블록 4개 ≈ 1.40 MiB, 블록 하나 ≈ 359 KiB**, 파티션당 victim pqueue 최대 크기 8,192(= CB 전체스캔 비용의 상한).
- **워크로드 스케일 대조** (디바이스 44.86 GiB 기준):

  | 워크로드 | 라이브 데이터 | 총 쓰기 | 비고 |
  |---|---|---|---|
  | hotcold v7 | 31.0 GiB (69% 채움) | 160 GiB (3.6배) | GC 압박 충분, 메인 실험으로 적절 |
  | uniform | 0.59 GiB (1.3% 채움) | 146.5 GiB (3.3배) | 워킹셋이 매우 작음 — 아래 참고 |
  | filebench | 2.0 GiB (4.5% 채움) | 88 GiB (2.0배) | 보조용으로 적정 |

- **검증 1 — 데이터 정합성(CRC)**: 그동안 fio/filebench 모두 쓰기만 하고 되읽어 검증한 적이 없어서, victim 선택 로직을 고친 뒤 데이터가 깨지지 않는다는 근거가 전혀 없었음. Cost-Benefit 정책으로 8 GiB 파일에 48 GiB(용량 초과 → GC 다수 발생)를 쓰며 CRC32C를 심고 전부 되읽어 검증(`fio --verify=crc32c --verify_fatal=1`) → **`err= 0`, 불일치 0건으로 통과.**
- **검증 2 — uniform 수렴의 원인 규명 (가장 큰 수확)**: uniform 실험은 진단 기능이 없던 구버전 빌드로 측정한 거라, "후보 대부분이 vpc=0이라 두 정책이 같은 선택을 한다"는 설명이 추론에 머물러 있었음. 최종 빌드로 재측정(`uniformdiag`, policy 0/2) 결과:
  - `total_gc = 67,905` (두 정책 완전 동일)
  - `greedy_vs_cb_identity_diverge = 0` — **67,905번의 GC에서 두 정책의 선택이 단 한 번도 갈리지 않음**
  - `avg_greedy_vpc = avg_cb_vpc = 0.000`, `gc_migrate_pages = 0` — **모든 victim이 vpc=0이라 옮긴 valid page가 하나도 없음(GC 비용이 문자 그대로 0)**
  - → uniform의 수렴은 "비슷해서"가 아니라 **워크로드가 강제한 구조적 필연**임이 실측으로 확정됨. hotcold(divergence 90.3%, `avg_abs_vpc_diff` 33.8)와의 대비가 리포트의 핵심 논지가 됨(fig8 신설).
  - 부수 확인: 앞서 기하 구조로 계산한 "erase 271,620 ÷ line당 블록 4 = line 회수 67,905"가 커널이 독립적으로 센 `total_gc`와 **정확히 일치** → erase 집계가 물리 구조와 정합함.
- **지표 개선 — `erase_cv_all` 신설**: 기존 `erase_cv`는 erase가 1회 이상 발생한 블록만 대상이라 정책마다 모집단 크기가 달라(Greedy 85,624 vs CB 89,433) 엄밀한 비교가 아니었음. 전체 131,072 블록 기준으로 다시 계산하니 모집단이 고정되면서 결론이 더 뚜렷해짐:

  | 정책 | 기존(nonzero 기준) | 전체 블록 기준 |
  |---|---|---|
  | Greedy | 0.2387 | 0.7861 ± 0.0017 |
  | Cost-Benefit | 0.2310 | **0.7375 ± 0.0045** |
  | Random | 0.4891 | 0.9369 ± 0.0031 |

  Greedy 최저(0.7843)와 CB 최고(0.7419)가 겹치지 않음 → "CB가 마모를 더 고르게 분산시킨다"는 주장이 훨씬 명확해짐. 두 실험 스크립트에 `erase_cv_all` 출력 추가(앞으로의 run에는 자동 포함). uniform은 8.26으로 hotcold(0.74~0.94)의 10배 이상 — 워킹셋이 작아 마모가 극단적으로 몰린다는 점도 수치로 드러남.
- **남은 정합성 이슈 (내일 처리 예정)**: 현재 리포트는 서로 다른 빌드에서 나온 데이터를 섞어 쓰고 있음 — hotcold(`vpcdiag`)는 최종 빌드지만 uniform 3회분과 filebench는 migration 카운터·진단이 없던 구버전 빌드 결과. **hotcold는 이미 최종 빌드 + 3회 반복 + 교차검증까지 끝났으므로 재실행 불필요**(재실행하면 그래프/표만 다시 맞춰야 하고 얻는 게 없음). uniform과 filebench만 최종 빌드로 다시 돌리면 리포트 전체가 한 버전으로 통일됨.

### 2026-07-31 (uniform/filebench 최종 빌드 재실행 — 리포트 버전 통일, 제출 준비)
- **uniform 최종 빌드 재실행** (`final31_rep1/2/3`, 정책 3종×3회, 총 9회): 이전 uniformdiag(policy 0/2, 1회씩)에서 봤던 "구조적 수렴"이 3회 반복 전부에서 그대로 재현됨. Greedy·Cost-Benefit 둘 다 3회 모두 완전히 동일: `sum=271620, max=161, gc_migrate_pages=0, total_gc=67905, diverge=0`. Random만 다름: `sum=273380`(3회 완전 동일), `max=9~10`(9,10,9), `gc_migrate_pages≈170,670~171,211`. latency avg/p99는 세 정책이 겹치는 범위(avg 34.5~35.0μs, p99 67~70μs)라 uniform에서는 정책 구분 지표로 못 씀 — 이전 결론 그대로 재확인.
- **LaTeX vs Markdown 질문**: 보고서를 Word로 변환하기 편하려면 LaTeX으로 써야 하는지 질문받음 → 답은 반대: 우리 리포트엔 복잡한 수식/전용 패키지가 없어서, 지금 있는 Markdown(`REPORT.md`)을 `pandoc`으로 바로 `.docx` 변환하는 게 가장 간단함(LaTeX→docx가 오히려 서식 깨짐 위험이 큼). pandoc은 서버에 미설치 상태, 필요시 `sudo apt install -y pandoc` 후 `pandoc report/REPORT.md -o report/REPORT.docx --resource-path=report`로 변환 가능.
- **filebench 최종 빌드 실행** (`final31`, 정책 3종×1회, 최종 빌드로는 처음): 실행 중 `WARNING: Could not open /proc/sys/kernel/shmmax file!` 경고가 떴으나, 로그에 바로 이어서 `Allocated 177MB of shared memory`가 정상 출력되고 `IO Summary`도 정상 완료돼서 무해한 경고로 확인(스크립트가 filebench 자체는 root 없이 실행하도록 짜여 있어서 매번 뜨는 경고이며, 요구하는 공유메모리 177MB가 현재 커널 기본 shmmax로 충분히 커버됨. 이전 `fbcalib`/`fbcalib2`(7/30) 로그에도 동일 경고가 있었고 그때도 문제없었음). 결과(ops×4096B로 정확히 GiB 환산 후 정규화): Greedy 87.34GiB/1355.3 erases-per-GiB/max 3/migrate 0, Cost-Benefit 88.76GiB/1379.7/max 3/migrate 0, Random 88.52GiB/1463.8/max 8/migrate/GiB 8590.8. **filebench에서도 Greedy·CB의 migration 비용이 둘 다 정확히 0** — uniform의 "스큐 없으면 구조적 수렴" 결론이 fio 외 도구에서도 재현됨.
- **리포트 갱신**: `report/make_figures.py`의 uniform(fig4, migration 패널 추가로 2패널→3패널)·filebench(fig5, migration 패널 추가로 3패널→4패널) raw 데이터를 위 최종 수치로 교체하고 전체 그래프 재생성. `REPORT.md` §4.2/§4.3의 표·본문을 갱신해서, 이제 리포트 전체가 하나의(최종) 빌드 데이터로 통일됨(이전까지는 hotcold만 최종 빌드, uniform/filebench는 구버전 빌드가 섞여 있었음).
- **최종 통독 검증**: `REPORT.md`의 모든 표 숫자를 원본 raw 데이터에서 파이썬으로 재계산해 전수 대조(hotcold 6개 지표, uniform, filebench, vpc divergence, 교차검증 표 등) — 전부 정확히 일치, 오탈자·논리적 비약 없음 확인. 변동계수 문장의 "0.742"가 반올림된 표값(0.738±0.005 → 얼핏 0.743)과 안 맞아 보여 재확인했으나, 반올림 전 원본 수치(0.7375±0.0045)로 계산하면 정확히 0.742가 나와서 오류 아님으로 판정(거짓 경보).
- **제출 파일 구조 확정**: `case_study` 커밋(실습 baseline, 7/23) 대비 실제 diff로 제출 대상을 가려냄 — `conv_ftl.c`/`conv_ftl.h`/`main.c`/`Kbuild`(핵심 구현) + `scripts/`(벤치마크 자동화 4개 파일) + `report/REPORT.md` + `report/figures/*.png`. 이 과정에서 **실수로 커밋된 산출물 2개를 발견**: `gc_diag.log`(7/30 힙 staleness 조사 중 dmesg printk를 그대로 캡처해둔 디버그 로그 덤프, 440줄)와 `local-verify-0-verify.state`(fio `--verify=crc32c` 실행 시 자동 생성되는 재개용 바이너리 상태 파일) — 둘 다 코드도 문서도 아닌 일회성 런타임 부산물이라 제출 목록에서 제외함. `EXPERIMENT_LOG.md`도 참고용으로 함께 포함.
- **제출 zip 생성**: 위 목록 그대로 `~/nvmevirt_submission.zip`으로 패키징(18개 파일, 약 1.1MB) — git 저장소 바깥(`$HOME`)에 생성해서 저장소에는 커밋 안 함.
