...
4. ~~**결과 저장 파이프라인**~~ → **완료 (2026-07-26)**: `scripts/run_experiment.sh <policy> <label>` (정책 전환→reset→fio json 출력→erase_cnt 덤프→summary 생성까지 한 번에) + `scripts/collect_summary.sh` (`results/` 전체를 CSV로 집계, latency는 fio json에서 jq로 추출)로 구현. 로컬 컴퓨터에서 Greedy로 1회 실측 검증 완료(`results/20260726_220135_policy0_greedy_pipelinetest`), 파일 4종(`fio.json`/`erase_cnt.txt`/`meta.txt`/`summary.txt`) 정상 생성 및 집계 확인.
5. **모듈 리로드해도 SSD 파일 데이터가 안 지워지는 문제 반영** (2026-07-24 발견): `mkfs`를 정책 비교마다 다시 할지, 아니면 "기존 파일 재사용"으로 통일할지 정할 것. 7/24 확인한 바로는 **같은 조건(기존 파일 재사용)을 유지하면 Greedy 결과가 완벽하게 재현됨**(3회 연속 `19280/61008/4`로 동일) — 그러니 "매번 mkfs로 완전히 새로 시작" vs "기존 파일 재사용" 둘 중 하나로만 **일관되게** 하면 될 것 같고, 섞어 쓰는 게 문제. **업데이트 (2026-07-26)**: "완전히 새로 mkfs한 상태"로도 처음 실측해봄 — Greedy `19520/62176/4`로, 7/24 기준선(`19280/61008/4`)과 max만 같고 나머지는 다름. 즉 **"fresh mkfs 직후 첫 쓰기" 조건과 "기존 파일 재사용" 조건은 서로 다른 결과를 냄** (버그가 아니라 파일 블록 할당 위치 자체가 달라지는 것으로 추정) — 정책 간 비교 실험에서는 이 둘 중 하나로 반드시 통일해야 하고, 아직 어느 쪽으로 할지는 미결정.
